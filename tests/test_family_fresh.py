...
def test_family_model():
    ...
    try:
        ...
        with app.app_context():
            ...
            family = Family(name="全新家庭", invite_code="FRESH001")
            db.session.add(family)
            db.session.commit()

            # 验证基本字段
            assert family.id is not None, "Family ID 应该不为空"
            assert family.name == "全新家庭", "Family 名称应该正确"
            assert family.invite_code == "FRESH001", "邀请码应该正确"
            ...
            assert family_dict['name'] == "全新家庭", "字典中名称应该正确"
            assert family_dict['invite_code'] == "FRESH001", "字典中邀请码应该正确"
            assert family_dict['member_count'] == 0, "初始成员数应该为0"
            print("✅ Family to_dict 方法测试通过")

            # 测试3: 测试 Family-User 关系
            print("📝 测试3: Family-User 关系")
            user = User(username="fresh_user", nickname="新用户")
            ...
            assert user_dict['family_name'] == "全新家庭", "用户字典应包含家庭名称"
            ...
            try:
                # 尝试创建重复邀请码的家庭
                family2 = Family(name="另一个家庭", invite_code="FRESH001")
